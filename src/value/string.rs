...
impl TryInto<Bytes> for String {
    ...
    fn try_into(self) -> SerializeResult<Bytes> {
        ...
        let mut bytes = BytesMut::with_capacity(mem::size_of::<u8>() + mem::size_of::<u32>() + self.value.len());
        bytes.put_u8(marker);
        match self.value.len() {
            0..=15 => {}
            ...
            _ => return Err(SerializeError::new(format!("String length too long: {}", self.value.len())))
        }
...
mod tests {
    ...
    fn get_marker() {
        ...
        assert_eq!(tiny.get_marker().unwrap(), MARKER_TINY | tiny.value.len() as u8);
    ...
    fn try_into_bytes() {
        ...
        let normal_bytes = String::from("abcdefghijklmnopqrstuvwxyz".to_string()).try_into_bytes().unwrap();
        assert_eq!(normal_bytes, Bytes::from_static(&[0xD0, 0x1A, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69,
            0x6A, 0x6B, 0x6C, 0x6D, 0x6E, 0x6F, 0x70, 0x71, 0x72, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78, 0x79, 0x7A]));
        let special_bytes = String::from("En å flöt över ängen".to_string()).try_into_bytes().unwrap();
        assert_eq!(special_bytes, Bytes::from_static(&[0xD0, 0x18, 0x45, 0x6E, 0x20, 0xC3, 0xA5, 0x20, 0x66, 0x6C, 0xC3,
            0xB6, 0x74, 0x20, 0xC3, 0xB6, 0x76, 0x65, 0x72, 0x20, 0xC3, 0xA4, 0x6E, 0x67, 0x65, 0x6E]));
    }
}
